...
import { clampAlpha, setChecked, preventBubble, isDescendant,
		 make, show, hide, addGetter, average, extend, isUdf } from "./utility";
import { addEvent, MyEvent, Listener, MouseAction } from "./event";
import { Box } from "./geometry";
import { DragWidget } from "./transform";

export { Toggle, Slider, ColorPicker, ColorBox };
...
const Toggle = (function(){
	...
	return class {
		constructor(initialValue, options) {
			...
			text.innerText = this._options.text;
			d.appendChild(text);
...
const Slider = (function(){
	...
	return class {
		constructor(initialValue, min, max, options) {
			...
			span.innerText = this._options.text;
			span.classList.add(cl_slider.text);
			d.appendChild(span);

			const input = make("input");
			input.style.direction = this._options.reverse ? "rtl" : "ltr";
			input.type = "range";
			input.min = min;
			input.max = max;
...
const ColorPicker = (function(){
	...
	return class {
		...
		_createDragWidget() {
			this._box = new Box(this._root, document);
		...
		_resetColor() {
			this.color = this._options.defaultColor;
		}
