...
import { MouseAction, MouseActionHandler } from "./event";
import { Vector2, Box } from "./geometry";
import { DragCommand, ResizeCommand, RotateCommand } from "./defaultCommands";

export { TestTool, MoveTool };
...
class SelectWidget extends MouseActionHandler {
	...
	_onClick(mdEvt, muEvt, layer) {
		if (layer) {		
			...
			if (this._layerManager.selected.length > 1) {
				this._layerManager.deselectAll();
			}
		}
	}
...
const BoxWidget = (function(){
	const DEFAULTS = { boxes: () => [], offStackBoxes: () => [], origin: Vector2.zero, scale: 1 };

	return class extends MouseActionHandler {
		constructor(stack, options) {
			super();

			let ef = new Enforcer(BoxWidget, this, "BoxWidget");
			ef.enforceAbstract();
